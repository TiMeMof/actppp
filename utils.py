...
class EpisodicDataset_ee(torch.utils.data.Dataset):
    def __init__(self, dataset_path_list, camera_names, norm_stats, episode_ids, episode_len, chunk_size, policy_class):
        super(EpisodicDataset_ee).__init__()
        self.episode_ids = episode_ids
        self.dataset_path_list = dataset_path_list
        self.camera_names = camera_names
        self.norm_stats = norm_stats
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # construct observations
            image_data = torch.from_numpy(all_cam_images)
            qpos_data = torch.from_numpy(ee).float()
            action_data = torch.from_numpy(padded_action).float()
            is_pad = torch.from_numpy(is_pad).bool()
...
def get_ee_norm_stats(dataset_path_list):
    all_episode_len = []
    all_ee_data = []
    all_action_ee_data = []
        ...
        try:
            with h5py.File(dataset_path, 'r') as root:
                ...
                if '/base_action' in root:
                    ...
                    #
                    action_ee = root['/action_ee'][()]
                    dummy_base_action = np.zeros([action_ee.shape[0], 2])
                    action_ee = np.concatenate([action_ee, dummy_base_action], axis=-1)
        except Exception as e:
            print(f'Error loading {dataset_path} in get_norm_stats')
            print(e)
            quit()
        all_ee_data.append(torch.from_numpy(eepos))
        all_action_ee_data.append(torch.from_numpy(action_ee))
        all_episode_len.append(len(eepos))
    all_ee_data = torch.cat(all_ee_data, dim=0)
    all_action_ee_data = torch.cat(all_action_ee_data, dim=0)

    # normalize action data
    action_mean = all_action_ee_data.mean(dim=[0]).float()
    action_std = all_action_ee_data.std(dim=[0]).float()
    action_std = torch.clip(action_std, 1e-2, np.inf) # clipping

    # normalize qpos data
    ee_mean = all_ee_data.mean(dim=[0]).float()
    ...
    stats = {"action_mean": action_mean.numpy(), "action_std": action_std.numpy(),
             "action_min": action_min.numpy() - eps,"action_max": action_max.numpy() + eps,
             "qpos_mean": ee_mean.numpy(), "qpos_std": ee_std.numpy(),
             "example_qpos": eepos}
